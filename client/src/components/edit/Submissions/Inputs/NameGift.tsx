...
const NameGift = () => {
  return (
    <div style={{marginTop: "10px"}}>
        ...
        <ValidationTextFields inputId={"nameGift"} name="nameGift" placeholder={"שם התשורה"} customCondition={(value)=> value.length > 0 && value.length <= 60} customConditionLogic={(value)=> value.length > 60} textError={'הטקסט שהוכנס חורג ממגבלת התוים לשדה זה'}/>
    </div>
  )
}
